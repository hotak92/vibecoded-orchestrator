...
- **Claude Code CLI** (`claude` binary): Per-project env via `.claude/env`
  (auto-generated by the launcher). Source it from your shell rc or use
  the bundled `tools/claude` wrapper which sources it for you before
  exec'ing the real `claude` binary.
- **VS Code extension**: Per-project env via `.vscode/settings.json`'s
  `claude-code.env` block (auto-generated by the launcher).
- **Claude Desktop app**: MCP servers connect via `~/.claude.json`.
  Per-project env: same approach as the CLI (use `.claude/env` + the
  wrapper).

When the launcher creates a project it writes BOTH `.vscode/settings.json`
and `.claude/env`, so you can switch surfaces freely without
reconfiguring.
