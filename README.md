...
| You set up the same `.claude/` config and hooks in every new project | An installer that drops 31 automation hooks + 53 skills + 45 agents into `.claude/`, wires the MCP servers, and stays out of your way |
...
- **31 automation hooks** — context injection on prompt submit, KG/code-graph auto-sync on file edit, credential scans, compaction-preserving context replay, security checks. Linux/macOS use `.sh`; Windows ships native `.ps1`. Plus the `vct-hub` background service that resolves per-project config for hooks, MCPs, and scripts.
- **4 MCP servers (default install)** — `weaviate-kg` (semantic + graph search + code graph), `search` (academic papers via OpenAlex + arXiv), `mermaid` (diagram describe/extract), `excalidraw` (diagram describe/extract). All local, no per-tool API keys. Authoritative writer: `launcher/src-tauri/src/mcp_registration.rs::build_default_mcp_entries` (pure-Python fallback at `install.py:20654-20661`). The `playwright` browser-automation MCP is **default-enabled but registered separately** via `npx -y @playwright/mcp@latest` (install.py pre-caches it at `_install_playwright_browsers`; opt out with `VCT_SKIP_PLAYWRIGHT=1`). The `vct-coordination` MCP is **Pro-tier** and excluded from the default install. Ollama runs as backend infrastructure (Weaviate vectorizer + code-embed CPU fallback) regardless of MCP wrappers; the CodeSage code-embedding FastAPI service runs locally on port 11440.
...
| User-extensible (hooks / agents / skills) | Yes (hooks + skills, no marketplace) | No | Limited (MCP only) | No | Limited (skills as prompts) | Yes (open source) | Yes (open source) | **Yes (31 hooks, 53 skills, 45 agents)** |
...
vibecoded-orchestrator/
├── .claude/
│   ├── hooks/                 # 31 automation hooks (.sh + .ps1 per hook)
...
|-----------------|----------------------|-----------------------------------------------------------------------------------------------|
| **Free**        | €0                   | Full orchestrator: KG, code graph, 31 hooks, 4 default MCP servers (weaviate-kg + search + mermaid + excalidraw) + playwright via npx, 45 agents, 53 skills. AGPL-3.0.   |
