...
if [[ "$TOOL_NAME" == "Read" ]]; then
    ...
    if [[ -n "$FILE_PATH" ]]; then
        # Build Anchor ledger (unchanged path for back-compat).
        echo "$FILE_PATH" >> "$SESSION_READS_FILE" 2>/dev/null || true
        # v0.2.70 Stream E: also record into the unified reads store the
        # injectors consult so a source the model Read explicitly isn't
        # re-injected. Uses an ABS path so the injectors' src match is exact.
        # Skipped when the session id is untrustworthy (helper returns "").
        if command -v vco_seen_store_path >/dev/null 2>&1; then
            _ABS_FP="$FILE_PATH"
            case "$_ABS_FP" in
                /*) : ;;  # already absolute
                *)  _ABS_FP="$(cd "$(dirname "$FILE_PATH")" 2>/dev/null && pwd)/$(basename "$FILE_PATH")" 2>/dev/null || _ABS_FP="$FILE_PATH" ;;
            esac
            _UNIFIED_READS="$(vco_seen_store_path reads "$SESSION_ID_RAW" "$PROJECT_ROOT")"
            if [ -n "$_UNIFIED_READS" ] && [ "$_UNIFIED_READS" != "$SESSION_READS_FILE" ]; then
                printf '%s\n' "$_ABS_FP" >> "$_UNIFIED_READS" 2>/dev/null || true
            fi
        fi
        ...
        if [[ "$FILE_PATH" =~ \.(py|js|mjs|jsx|ts|tsx|go|rs|lua|cpp|cc|cxx|c|h|hpp|java|rb|cs|proto|sh|bash)$ ]]; then
            ...
            _cg_inject "$_RD_Q" "$FILE_PATH" "Code-graph context for $(basename "$FILE_PATH")"
        fi
    fi
    exit 0
fi
...
# Gated by the shared codegraph_symbol_gate (dotted / CamelCase / name( / code
# path); a bare-word pattern like "TODO" does NOT fire. EXCLUDES nothing extra —
