...
def analyze_go_file(ctx: Any, file_path: Path, repo_root: Path) -> Dict[str, int]:
    ...
    for sname, start_line in struct_info.items():
        ...
        ctx.store_entity(CodeEntity(
            ...
            deferred_embed=lambda: ctx.embed_class(signature, class_body, language="go"),
        ))
        stats['classes'] += 1
    ...
    for m in func_pattern.finditer(content_clean):
        ...
        ctx.store_entity(CodeEntity(
            ...
            references={"module": module_uuid},
            deferred_embed=lambda: ctx.embed_function(signature, body, language="go"),
        ))
        stats['functions'] += 1
    ...
    if ix:
        stats['interactions'] = ctx._store_interactions(ix, "Go", module_uuid, file_path_rel=relative_path)
