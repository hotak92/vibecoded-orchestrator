...
def analyze_java_file(ctx: Any, file_path: Path, repo_root: Path) -> Dict[str, int]:
    ...
    for cname, start_line in class_info.items():
        ...
        ctx.store_entity(CodeEntity(
            ...
            deferred_embed=lambda: ctx.embed_class(signature, class_body, methods=methods[:10], language="java"),
        ))
        stats['classes'] += 1
    ...
    for m in method_pattern.finditer(content_clean):
        ...
        ctx.store_entity(CodeEntity(
            ...
            references={"module": module_uuid},
            deferred_embed=lambda: ctx.embed_function(signature, body, language="java"),
        ))
        stats['functions'] += 1
    ...
    if ix:
        stats['interactions'] = ctx._store_interactions(ix, "Java", module_uuid, file_path_rel=relative_path)
