...
echo ""
echo "============================================================"
echo " v0.2.44 pre-ship gate check"
echo " Repo: $REPO"
...
# Gate 1: Cargo.toml version matches expected v0.2.44
CARGO_VER="$(grep -m1 '^version = ' launcher/src-tauri/Cargo.toml \
    | sed -E 's/^version *= *"([^"]+)".*/\1/')"
if [ "$CARGO_VER" = "0.2.44" ]; then
    gate_pass "Cargo.toml version = 0.2.44"
else
    gate_fail "Cargo.toml version = 0.2.44" "Found: $CARGO_VER (bump before tagging)"
fi
...
#
# v0.2.44: this gate is WARN-level, not FAIL-level. Reasoning:
...
# Gate 16: CHANGELOG has v0.2.44 entry
# Match the Keep-a-Changelog heading shape: `## [0.2.44] — 2026-06-01`.
# Allow optional `v` prefix and optional surrounding brackets for flexibility.
if grep -qE '^## \[?v?0\.2\.44\]?' CHANGELOG.md 2>/dev/null; then
    gate_pass "CHANGELOG.md has v0.2.44 section"
else
    gate_fail "CHANGELOG.md has v0.2.44 section" \
...
if [ "${#failures[@]}" -gt 0 ]; then
    ...
    echo ""
    echo "Fix all failed gates before tagging v0.2.44."
    exit 1
fi

echo "All gates PASSED. Safe to tag v0.2.44."
exit 0
