...
(No unreleased changes — v0.2.53 is the latest tag.)
...
- **Podman machine auto-init on macOS + Windows** (`install.py:7036`): closes M-P1-2. install.py now detects the "Podman binary installed but machine never initialized" state and offers to run `podman machine init --memory 4096 --cpus 2 --disk-size 30 && podman machine start` interactively (or runs it under `--yes`). Pre-fix: install.py's container probe required `podman info` to succeed, which fails when the machine isn't running, so the install would report "no container runtime" despite Podman being installed.
...
- **NEW-11 resume sentinel empty-sha refusal** (`launcher/src-tauri/src/commands/installer.rs:6255`): `resume_orchestrator_update` now refuses to resume when the sentinel's `sha_at_conflict` is empty. Pre-fix: an empty-string fallback could let resume proceed despite HEAD not having advanced past the conflict SHA — defensive hardening for Fabio Symptom A even though Symptom A was determined to be most likely user-error rather than this code path.
