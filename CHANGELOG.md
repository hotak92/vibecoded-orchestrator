...
- **V52-AH — Windows binary lock during update** (DEFERRED to v0.2.53 by EXPLICIT user scope decision 2026-06-09): the architectural fix (Velopack/Squirrel-style stage1 updater.exe spawn → wait-for-PID-exit → atomic-replace → relaunch) is too large for v0.2.52's ship window. v0.2.52 ships a workaround in `docs/TROUBLESHOOTING.md` (kill launcher+hub, `git checkout dist/{linux,macos,windows}-*/{vct-launcher,vct-hub}{.exe,}` — git has the freshly-pulled binaries — relaunch). Reported by Fabio msg 267 (Bug 1, 2026-06-09); reproducible on Windows 24GB-no-GPU.
<<<<<<< HEAD
=======
- **V52-AI — MCP fork-bomb during update on Windows** (DEFERRED to v0.2.53 by EXPLICIT user scope decision 2026-06-09): pre-update gate + lockfile design requires V52-AH's stage1 updater pattern to land first; tracked as a follow-up. v0.2.52 ships no mitigation other than the Bug 1 workaround (which kills MCP children as a side-effect of the manual launcher+hub kill).
...
>>>>>>> 2f323039 (docs(changelog): Known Issue 6 entry under [Unreleased] Fixed)
