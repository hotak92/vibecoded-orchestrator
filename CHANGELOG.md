...
- **V52-S — Trainability threshold constants + check script**: lifts the four corpus-health thresholds (citation pair rate 30%, n_emb presence 95%, query_emb presence 99%, cohort uniformity 95%) from A2's audit prose into `vco_lib/rl_trainability_thresholds.py` as named module-level constants. New `scripts/trainability_check.py` reads launcher.db's `rl_events`, computes the four metrics, and emits a per-metric PASS/FAIL plus an overall TRAINABLE / NOT_TRAINABLE_AS_IS verdict (supports `--json` for machine consumption). 6 tests cover constants + synthetic-corpus passing/failing verdicts + missing-DB + empty-corpus edge cases.
- **V52-T — `tests/test_v52_i_post_deploy_smoke.py`**: post-deploy canary that probes `launcher.db rl_events` for `partial_fan_out_schema_missing` failure-mode events written AFTER the v0.2.52 tag time. Zero rows expected within 48h post-tag; any row indicates V52-I's defensive filter has regressed. Auto-skips until the operator fills in `V0252_TAG_TIME_MS` per the post-deploy runbook.

### Changed

- **V52-Y — SubagentStart hook timeout 2s → 5s**: bumps `templates/settings.json.{linux,windows}.template` SubagentStart hook timeout from 2s to 5s. The hook's cold path (Python venv warmup + Weaviate connect for keyword matching) sometimes exceeded 2s on first invocation, causing SIGKILL and silently dropping agent/skill suggestions. 5s comfortably covers the observed 1.5-2.5s typical cold latency.

### Fixed

- **V52-Z — extra-path runs upsert `code_graph_builds`**: the launcher's "last successful build" UI reads `code_graph_builds.finished_at`, but the two extra-path code paths (`sync_project_codegraph_extra_path` and `reindex_project_codegraph_after_extras_change`) updated `project_codegraph_extra_paths.last_indexed_at` and skipped `code_graph_builds` entirely. UI timestamps lagged real activity by hours/days whenever extra-path syncs dominated (orchestrator-root last analyzed 2026-06-05 via extra-path but UI showed 2026-05-18). Both completion sites now upsert `code_graph_builds` with the analyzer's reported counters. Best-effort: row-write failures log but don't fail the user-visible sync.
...
- **V52-Q + V52-R — closed by V52-J Edit 3 (already shipped in commit `ab706bd`)**: the `score_cosine` per-node field (V52-Q) is now attached at the Weaviate fetch sites in both `hybrid_search` (`claude_mcp_servers/weaviate_mcp/server.py:5452-5460` + `5503-5505`) and `semantic_graph_search` (`server.py:5140-5150`) via `r["score_cosine"] = 1.0 - r["distance"]` — distinct from the fused hybrid `score` which can exceed 1.0. The `emb` per-node field (V52-R) is attached via `_extract_obj_vector(obj, query_target)` at the same fan-out sites (`server.py:5158-5160` for SGS, `5468-5470` for HS). The downstream reducer `_build_log_nodes` in `claude_mcp_servers/rl_client/search_pipeline.py:369-414` already preserves both fields into the v3 telemetry envelope. Closed; CHANGELOG note for the audit trail.
- **V52-U — closed by V52-O.11.K**: `_ensure_project_source_property` adds the `project_source` field to CodeFunction/CodeClass/CodeModule schemas but doesn't back-fill values for pre-v0.2.47 rows (163 of 397 CodeModule rows ship `project_source=""`). V52-O.11.K's collection drop-and-rewalk subsumes this — the legacy empty-string rows vanish on re-walk. No separate fix needed; same delete closes both items.
- **V52-W — deferred to V52-D.2 reaper (v0.2.53)**: the `module_installs` row for `vct-rl-reranker` on orchestrator-root still carries a stale `last_error='rl-latest-weights returned 500 Internal Server Error'` from the v0.2.46 schema-cache-miss incident (fixed by the `paid_module_releases` migration deploy). The cosmetic stale error in the GUI will self-heal when V52-D.2's container reaper successfully restarts the container and clears `last_error` for the row. V52-D.1 (the image-variant + authfile fix) shipped in this release; V52-D.2 (the reaper) is tracked for v0.2.53. ~3 LoC follow-up in the reaper logic.
- **V52-X — deferred to V52-D.2 reaper (v0.2.53)**: the `module_installs` row `b7e3265e` (project SD15) carries a stale `container_name='vct-rl-reranker-manual'` artefact left over from a manual debugging session — never matched a real container and caused the respawn loop documented in V52-D.2's plan. The reaper's recreate-from-canonical-manifest logic will overwrite this self-healingly when it lands; no separate fix needed in this release.
