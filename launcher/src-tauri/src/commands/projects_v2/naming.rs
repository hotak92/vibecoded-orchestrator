...
/// stale value as its active line. This decides the NAMES (the launcher's
/// sanitizer lives here) and hands the rewrite to the ONE `.env` writer,
/// `python -m vco_lib.env_template repair-kg` (v0.2.97 — it was a Rust
/// read-modify-write the single-writer lint could not see): the first stale
/// line is rewritten in place, annotated with its previous value, every
/// other byte kept.
///
/// Returns:
/// - `Err(message)` if the writer could not run or could not rewrite.
...
pub fn b12_repair_stale_kg_collection(
    ...
    let action = crate::services::vco_lib_bridge::repair_project_env_kg(
        root,
        folder,
        &canonical_kg,
        &["KnowledgeGraph", kg_basename.as_str()],
    )?;
    Ok(if action == "repaired" {
        B12Outcome::Repaired { canonical_kg }
    } else {
        B12Outcome::NoChangeNeeded
    })
}
