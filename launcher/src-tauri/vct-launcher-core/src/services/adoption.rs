...
//!
//! v0.2.62: this module MOVED here from
//! `launcher/src-tauri/src/services/adoption.rs` (which is now a thin
//! `pub use` re-export). It is the SINGLE source of truth for the
//! `services.toml` schema. Two consumers now read it:
//!   * the launcher GUI (`commands::lifecycle`, `services::watcher`, …)
//!     — writes the user's choice and renders it;
//!   * the hub-side infra watchdog (`vct-hub::infra_watchdog`) — reads
//!     it every tick so it NEVER restarts a service the user adopted
//!     (Adopt), runs its own copy of in parallel (Parallel), or
//!     explicitly opted out of (Refuse). Only `Unresolved` (the default,
//!     i.e. no entry / a VCO-managed service) is eligible for an
//!     auto-restart.
//! Keeping ONE copy avoids the schema drift the v0.2.47
//! supervisor-image-resolution-variant-gap bug taught us to fear.
