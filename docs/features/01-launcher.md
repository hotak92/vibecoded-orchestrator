...
### Preferences → Startup: two switches, two different events
`/preferences` carries a **Startup** section with two toggles. They are deliberately separate: the events, the stores and the failure modes all differ, and one generalised "startup" control would have to branch internally on which service it meant.

**"Start background service on login"** (default OFF) registers/unregisters the `vct-hub` boot-autostart unit (cross-OS: systemd-user / launchd / Windows Scheduled Task) via `get_hub_boot_autostart` / `set_hub_boot_autostart`, wrapping `hub_status::{boot_status, register_boot, unregister_boot}`. It starts the background hub (not the launcher GUI) on login, so hooks / MCPs / scripts reach a resolver even before the GUI is opened. Tri-state: **enabled** / **disabled** / **unsupported** — when the host's init system can't be inspected the toggle is disabled with a hint rather than claiming a state it can't honor.

**"Start the launcher with a Claude Code session"** (v0.2.95, ruling R2 — default **ON**) is the launcher GUI's own switch, and it fires when you open a project rather than when you log in: the `session-start-ensure-hub` hook (Claude Code's `SessionStart`, and `.vscode/tasks.json` on VS Code's `folderOpen`) calls `python -m vco_lib.launcher_ensure ensure`, which starts this binary with `--start-hidden` if no launcher is running. Backed by `commands::session_autostart::{get,set}_launcher_session_autostart` over the `app_state` key `launcher.session_autostart` — a row the launcher WRITES and never reads, because its reader is a different process at a moment when this one is by definition not running. Binary rather than tri-state: unlike boot registration there is no host capability to inspect. No row means ON, which is how an existing install picks the behaviour up after an update without a migration. Details, including the per-OS "created hidden" mechanism and the `VCT_DISABLE_LAUNCHER_AUTOSTART` kill switch, are in `docs/CONFIGURATION.md`.
...
The gateway is a **process**, not a container, so the hub supervises it in its own task (`vct-hub/src/gateway_watchdog.rs`) rather than through the infra-container watchdog — whose service list is an allowlist where every name reaches a `compose up`. It probes `/health` on the resolved port (env pin → the daemon's port file → the launcher's last-started-port record → 11436) every 30 s, and on a miss spends one of **3 attempts per 10 minutes** on `python -m vco_lib.gateway_ensure ensure --json` — the same entry point the SessionStart hook calls, so the per-OS start sequence (systemd-user / launchd / Scheduled Task) exists once, in Python. The budget is tighter than the shipped systemd unit's own `StartLimitBurst`, because the heal path calls `reset-failed` and would otherwise clear it forever; when it is spent the hub stops, records the reason in launcher.db (`app_state` + an `audit_log` row) and the Services card renders it, since a detached hub's stderr reaches nobody. It leaves alone: a gateway that is not registered (the opt-in is the user's), one whose registration cannot RUN (reported, never restart-looped), a process its own pid guard reports alive (nothing this supervisor did not start is ever signalled), and a port answering with a foreign service. Opt out with `VCT_HUB_GATEWAY_WATCHDOG=0`; there is nothing to register — the supervisor ships inside the hub binary that `install.py --update` refreshes.

The Services card reads the registration from the same Python home and renders three states rather than two: **not registered** / **registered but unrunnable** (with the reason, the artefact path and the repair command) / **running**. That third state is what a machine sat in for eight hours on 2026-09-10 while the toggle said "registered".
...
v0.2.95 removed `StateFlags::VISIBLE` from `tauri-plugin-window-state`'s flag set, which changes how those preferences behave in two ways worth knowing. That plugin ends its restore with `show()` **and** `set_focus()` whenever VISIBLE is set — so `tray_start_minimized` could previously only hide a window the plugin had just put in front of the user, and every ordinary launch grabbed focus on its own account. It also persisted the last visibility, so quitting from the tray while hidden made the next launch open no window at all. Window visibility now has one home — the window configuration, as amended by `--start-hidden` — while size, position, maximized, decorations and fullscreen still restore.
