...
**Claude Orchestrator** — a workflow engine for Claude Code: four MCP servers (semantic search, local inference, web search, code embeddings), a Weaviate-backed knowledge graph, an AST-extracted code graph, 19 free agents, 28 skills, and 20 hooks. Gives Claude Code persistent memory, free local inference, and a place to put automation that would otherwise live in shell aliases.
...
| [02-mcps-and-agents.md](02-mcps-and-agents.md) | Four MCP servers (Weaviate-KG, Ollama, Search, code-embed service), KG/code-graph scripts | ~65 |
| [03-agents-skills-hooks.md](03-agents-skills-hooks.md) | 19 free agents, 28 skills, 20 hooks, composition patterns | ~75 |
...
| **MAO** | MultiAgent Orchestrator — the full multi-agent stack (separate Pro product, not in OSS bundle) |
