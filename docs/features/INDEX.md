...
**VibeCoded Orchestrator (VCO)** — a workflow engine for Claude Code: four default MCP servers (`weaviate-kg` semantic + graph search, `search` for academic papers via OpenAlex + arXiv, `mermaid` + `excalidraw` for diagram describe/extract) plus `playwright` invoked via `npx -y @playwright/mcp@latest`. Backed by a Weaviate knowledge graph, an AST-extracted code graph, 45 free agents, 53 skills, and 31 hooks. Gives Claude Code persistent memory, free local inference, and a place to put automation that would otherwise live in shell aliases.
...
| [02-mcps-and-agents.md](02-mcps-and-agents.md) | Four default MCP servers (`weaviate-kg`, `search`, `mermaid`, `excalidraw`) + `playwright` via npx, code-embed service, KG/code-graph scripts | `_lib/update_gate.py` MCP-side mirror of `vco_lib.update_gate`; Track F two-layer case-rebind closes Fabio Symptom B |
