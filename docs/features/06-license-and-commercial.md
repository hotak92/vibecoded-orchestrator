...
`appId` is always set (controls which app entry appears in `profiles.apps`). `tier` is only set when `appId === "orchestrator"`. This allows non-orchestrator LS products (Ecosystem App 1, Ecosystem App 2, etc.) to use the same webhook without polluting the orchestrator tier.
