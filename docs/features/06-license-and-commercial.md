...
### Pro tier
Adds: `rl_retrieval`, `auto_update`, `curated_agent_packs`, `watermark_disabled`. Requires a valid Lemon Squeezy license key mapped to a Pro variant.
...
On first successful auth, `lookupVaultAdminToken` returns `bind_machine_hash` set to the requesting machine's sha256-hashed MAC; the edge function then calls `bindVaultAdminMachine` to write it back into the Vault map. Subsequent auths from a different machine return `outcome="machine_mismatch"` (401 + audit log entry). Trade-off: legitimate machine changes require a manual rebind by the project owner. Threat model: a leaked token can't be used from another machine after first activation.
...
Collects: OS name/version, Python version, orchestrator version, machine hash (sha256 of MAC), license tier and validity status (NOT the key itself), hashed session ID.
...
Until `https://api.vibecodedtools.it/telemetry` is deployed, opted-in events are written to `~/.vibecoded/telemetry_pending.jsonl` (one JSON object per line) instead of POSTed. `UploadResult.error == "endpoint_pending_deployment"` signals this path.
