...
### Packages from dev venv NOT in vibecoded closure

The Claude Orchestrator development venv contains a few packages with concerning licenses that are NOT part of vibecoded-orchestrator's dependency closure:

- `ultralytics`, `ultralytics-thop` (AGPL-3.0+) — YOLO computer vision, not used by vibecoded
- `pyinstaller` (GPL-2.0-only) — packaging tool, not used
- `pyphen` (triple LGPL/GPL/MPL) — hyphenation, not used

These are flagged in this audit only to confirm they do NOT leak into vibecoded's shipped artifacts.
