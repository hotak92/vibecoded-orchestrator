...
`DUAL_EMBEDDING_WRITE_ALL_SLOTS` and `DUAL_RL_LOG_ENABLED` must both be truthy for a dual (other-slot) event to be written. Chunk boundaries always follow the **active** model's own preset — dual-write never changes the active slot's chunk fidelity. When a chunk exceeds a secondary model's `num_ctx`, only that secondary's vector is computed over a bounded leading sub-window. The bound is per-model: `num_ctx` × the model's **measured minimum** chars-per-token ratio (arctic 2.30, qwen3 2.547; unmeasured models get the measured floor) with a 25% safety margin — sized so a budgeted input cannot overflow the window in true tokens even at the densest measured tokenizer ratio. A `prompt_eval_count` pinned at the window is not a truncation verdict: it means the window was **filled** — an exact fit and a truncation produce the identical number — and with `truncate: false` sent on every embed, an over-window input never returns 200 at all (the runner refuses it, and the caller shrinks and retries). Truncation is therefore known locally — a slot is tagged exactly when less text was sent than the chunk contained — and the char-ratio bound sizes the attempt, not the verdict.

The truncation fact is persisted on **two** surfaces:

- the Weaviate chunk row, which persists the complete record as `truncated_slots` (every configured slot, the active one included, that was sub-windowed) alongside `truncation_measured_slots` (the slots that row can answer for; an absent slot reads UNKNOWN, never False) — with the frozen back-compat `secondary_truncated_slots` view derived from it, and
- the per-node `emb_truncated` field on the dual-log (other-slot) retrieval event itself (`rl_events`, schema v4) — so truncated-vs-full secondary vectors can be partitioned from the RL event **alone**, with no Weaviate join.

`emb_truncated` is **tri-state**: `true` / `false` when the event knows the state (an explicit `false` is a real full-fidelity answer), and **absent = `unknown`** — for every event recorded before schema v4, for other-slot vectors that were backfilled on the fly at retrieval time, and for active-slot events, which carry no persisted truncation state. Readers resolve the state with `resolve_emb_truncation_state` (`rl_client/rl_logger.py`); a missing field must never be read as "not truncated", because the historical corpus predates the field and the actually-truncated rows are exactly the ones that would otherwise poison secondary-slot training. What training does with `unknown` rows is a trainer-side decision. A single-slot (non-dual) install keeps its single-model chunk sizing byte-for-byte.
