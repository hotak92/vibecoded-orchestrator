...
6. Copies 45 agent templates into `.claude/agents/` and 53 skill templates into `.claude/skills/`; renders 36 hooks (both `.sh` and `.ps1` per hook on every OS — cross-OS workflows don't get stale orphans) into `.claude/hooks/`
...
# fallback) — visible in `podman ps` / `docker ps`, no MCP wrapper. `vct-coordination`
