...
`install-smoke-tri-os.yml` runs the actual shims end-to-end on ubuntu-22.04, ubuntu-24.04, macos-14, windows-latest, and fedora-40 on every PR that touches an install surface, on every push to `main`, on manual `workflow_dispatch`, and — since v0.2.91 — as a `workflow_call` prerequisite of the Release workflow's `build` job, so a red smoke means no assets are built and no release is published. There is **no cron**: the daily 06:00 UTC schedule was removed on 2026-07-23 and must not be re-added. The Fedora job exercises SELinux `:Z` mount handling; the Ubuntu jobs exercise libwebkit2gtk-4.0/4.1 fallback; macOS exercises the Apple Silicon Homebrew cascade and bash 3.2; Windows exercises `first-install.bat` in real `cmd.exe` (the separate `installer-smoke.yml` Windows job runs under `shell: bash`, which never reaches the BAT code path). Pre-ship gate 22 blocks release tags when this workflow is red on main — and, since v0.2.91, also when its newest all-green run on `main` is older than 24 h, because without a cron a stale green is a likely state rather than a rare outage. The fix the gate asks for is a `workflow_dispatch` run (free on the public repo); the release-time `workflow_call` is a release blocker, not a freshness source (a `uses:`-called run executes inside the caller's run and never appears in `gh run list` for this workflow, and the gate reads before the tag exists).
...
2. `session-start-ensure-hub.sh` — three idempotent legs, each a call into its own `vco_lib.*_ensure` module: **hub** (`vco_lib.hub_ensure` — probes `vct-hub /health`, starts it if not running; the hub outlives the launcher GUI, so projects opened directly via the CLI or VS Code still get the resolver), **model gateway** (`vco_lib.gateway_ensure`, v0.2.95 — only when one is registered for this project) and **launcher GUI** (`vco_lib.launcher_ensure`, v0.2.95 — only when session-autostart is enabled, started hidden so it never steals focus). A leg that can do nothing reports and moves on; none of them can block the session
...
**On every explicit retrieval** (`PostToolUse` hook on `mcp__weaviate-kg__hybrid_search` / `semantic_graph_search` / `search_code_graph`, v0.2.91):
- `post-mcp-retrieval-record.{sh,ps1}` records what the retrieval already put in the context window into the same per-session stores the injecting hooks consult, so a node or entity you deliberately fetched is not re-injected minutes later by the pre-edit hook
- It records only what is provably in context byte-for-byte: a KG result's per-chunk key (and, for a `coverage: complete` result, the node's source path), and a code entity's `full_name` only when the result carried the untruncated `function_body` / `class_body`. A `titles`-detail search or a truncated entry records nothing
