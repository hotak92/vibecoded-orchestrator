...
# MUST-MATCH vco_lib/codegraph_schema.py: ``specs_subset(_V6_PROPS)`` — the
# class-name-suffix → ((prop, DataType name, description), ...) projection.
# CodeAPI / CodeInteraction are absent (no v6 props → never probed).
# Consumed ONLY by the inline fallback; parity is asserted by
# tests/test_codegraph_schema_parity.py.
_FALLBACK_SPECS = {
    "CodeModule": (("is_test", "BOOL", _IS_TEST_DESC),),
    "CodeClass": (("is_test", "BOOL", _IS_TEST_DESC),),
    "CodeFunction": (
...
def _ensure_props(client, prefix: str) -> dict:
    ...
    if ensure_fn is not None:
        return ensure_fn(client, prefix, props_subset=_V6_PROPS)
...
def main() -> int:
    ...
    try:
        try:
            ...
            # they will be born v6-shaped by create_collections.
