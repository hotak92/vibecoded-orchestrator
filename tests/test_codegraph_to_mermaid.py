...
class TestRenderMermaid:
    ...
    def test_byte_stable_golden(self) -> None:
        ...
        expected = (
            "%% [PRE-ALPHA] auto-generated by `vco codegraph-diagram`.\n"
            "%% Output may be incomplete, inaccurate, or visually broken.\n"
            "%% Verify against the source code before sharing or making "
            "decisions.\n"
            "---\n"
            "title: my title\n"
            "---\n"
            "flowchart TD\n"
            '    subgraph m_317ca0248f95 ["pkg.mod1"]\n'
            '        n_a["alpha()"]\n'
            '        n_b["beta()"]\n'
            "    end\n"
            '    subgraph m_a242f40a79e8 ["pkg.mod2"]\n'
            '        n_c["Gamma"]\n'
            "    end\n"
            "    n_a --> n_b\n"
            "    n_a --> n_c\n"
        ...
        # The golden comparison. Module IDs depend on the hash function, but
        # every CPython build ships the same hashlib, so the literal hashes
        # above are stable — and if the id scheme ever changes, THIS is the
        # useful failure point.
        assert actual == expected
        # Same render twice → byte-identical (the "stable across runs" half).
        assert cgm.render_mermaid(sg, title="my title") == actual
        # Spot-checks below keep the failure message readable.
