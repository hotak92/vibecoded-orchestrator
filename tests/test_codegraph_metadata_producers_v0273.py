...
def test_is_test_path_is_imported_not_mirrored(analyzer_mod):
    """v0.2.76 (R1): the analyzer's `is_test_path` IS the single-home
    `weaviate_mcp.code_ranking.is_test_path` object — imported directly, not a
    byte-identical inline mirror. The prior parity test
    (`test_is_test_path_parity_with_code_ranking`) locked TWO independent
    bodies; R1 deleted the inline fallback (loud-fail on a broken install), so
    there is nothing to keep in sync. Assert IDENTITY: if a future edit
    reintroduces a fallback body, this identity check fails and the drift risk
    is surfaced."""
    try:
    ...
    assert analyzer_mod.is_test_path is shared, (
        "analyzer.is_test_path must BE the imported code_ranking symbol (R1: "
        "direct import, no mirror) — a distinct object means a fallback body "
        "was reintroduced"
    )
