...
`weaviate_mcp` had the same trap one directory deeper, and it bit harder: the package lives under `claude_mcp_servers/`, which nothing put on the path, while an editable install's `_editable_impl_weaviate_mcp.pth` puts *another* checkout's `claude_mcp_servers` there at interpreter start. From a worktree that meant `vco_lib` came from your checkout and `weaviate_mcp` came from someone else's, in the same process. Since v0.2.94 `tests/conftest.py` pins both import roots (and `tests/common/child_env.py` pins them for spawned children), so the bare `pytest` above is safe; `tests/test_v0294_weaviate_mcp_imports_from_the_checkout.py` is the canary that fails, naming the path it got, if either pin stops working. The same check by hand: `python -c "import weaviate_mcp; print(weaviate_mcp.__file__)"`.
